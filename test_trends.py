...
def create_test_profile(manifest_path: str, date_offset: int = 0) -> ProjectRiskProfile:
    ...
    dependencies.append(
        DependencyRiskScore(
            ...
            factors=["severely outdated", "low maintenance", "missing security controls"]
        )
    )
    ...
    profile.high_risk_dependencies = sum(1 for dep in dependencies if dep.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL])
    profile.medium_risk_dependencies = sum(1 for dep in dependencies if dep.risk_level == RiskLevel.MEDIUM)
    profile.low_risk_dependencies = sum(1 for dep in dependencies if dep.risk_level == RiskLevel.LOW)
    
    # Set overall risk score
    profile.overall_risk_score = sum(dep.total_score for dep in dependencies) / len(dependencies)
    ...
    for i in range(num_profiles):
        ...
        if i > 0:
            ...
            if i == num_profiles - 1:
                ...
                profile.dependencies[2].dependency.security_metrics.has_security_policy = True
                profile.dependencies[2].dependency.security_metrics.has_dependency_update_tools = True
        
        # Recalculate overall profile metrics
        profile.high_risk_dependencies = sum(1 for dep in profile.dependencies if dep.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL])
        profile.medium_risk_dependencies = sum(1 for dep in profile.dependencies if dep.risk_level == RiskLevel.MEDIUM)
        profile.low_risk_dependencies = sum(1 for dep in profile.dependencies if dep.risk_level == RiskLevel.LOW)
        profile.overall_risk_score = sum(dep.total_score for dep in profile.dependencies) / len(profile.dependencies)
...
def main():
    ...
    parser.add_argument(
        ...
        "-g",
        action="store_true",
        help="Generate test historical data"
    )
