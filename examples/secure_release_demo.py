...
from dependency_risk_profiler.secure_release.code_signing import (
    SigningMode, sign_artifact, verify_signature
)
from dependency_risk_profiler.secure_release.release_management import (
    VersionBumpType, create_release
)
...
def demo_code_signing(output_dir: Path) -> None:
    ...
    print(f"TEST signature verification: {'✅ Success' if test_verified else '❌ Failed'}")
    
    release_verified = verify_signature(artifact_path, release_sig_path)
    print(f"RELEASE signature verification: {'✅ Success' if release_verified else '❌ Failed'}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--signing-only",
        action="store_true",
        help="Only demonstrate code signing"
    )
