...
def fetch_url(url: str, timeout: int = 30) -> Optional[str]:
    ...
    try:
        headers = {
            "User-Agent": "dependency-risk-profiler/0.1.0 (https://github.com/username/dependency-risk-profiler)"
        }
...
def check_for_vulnerabilities(package_name: str, ecosystem: str) -> bool:
    ...
    vulnerability_patterns = [
        r"security", r"vuln", r"CVE-\d{4}-\d+", r"advisory", r"exploit",
        r"malicious", r"remote code execution", r"injection", r"XSS", r"CSRF"
    ]
    
    return any(re.search(pattern, content, re.IGNORECASE) for pattern in vulnerability_patterns)