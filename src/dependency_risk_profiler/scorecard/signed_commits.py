...
def check_recent_commit_signature_status(repo_dir: str, commit_count: int = 20) -> Dict[str, int]:
    ...
    try:
        ...
        for line in output.split("\n"):
            ...
            try:
                _, status = line.split(" ")
                if status in ["G", "B", "R"]:  # Good signature (valid, but may be expired)
...
def check_release_signature_status(repo_dir: str, tag_count: int = 10) -> Dict[str, int]:
            ...
            for workflow_file in workflow_dir.glob("*.y*ml"):
                try:
                    with open(workflow_file, 'r', encoding='utf-8') as f:
                        ...
                        if re.search(r"signed(-|\s)commits?|commit(-|\s)sign(ing|er|ature)", content):
                            result["requires_commit_signing"] = True
                            result["commit_signing_mechanism"] = f"GitHub Actions workflow: {workflow_file.name}"
                            break
                except Exception:
                    continue
...
def calculate_signed_commits_score(
    ...
    if commit_signature_data["total_commits"] > 0:
        commit_signing_ratio = commit_signature_data["verified_commits"] / commit_signature_data["total_commits"]
    ...
    if tag_signature_data["total_tags"] > 0:
        tag_signing_ratio = tag_signature_data["verified_tags"] / tag_signature_data["total_tags"]
...
def identify_signed_commits_issues(
    ...
    if commit_signature_data["total_commits"] > 0:
        verified_ratio = commit_signature_data["verified_commits"] / commit_signature_data["total_commits"]
        
        if verified_ratio < 0.5:
            issues.append(f"Less than half of recent commits are signed ({verified_ratio:.0%})")
        
        if commit_signature_data["unverified_commits"] > 0:
            issues.append(f"{commit_signature_data['unverified_commits']} recent commits have problematic signatures")
    else:
        issues.append("No commit history available for signature verification")
    
    # Check tag signing issues
    if tag_signature_data["total_tags"] > 0:
        verified_ratio = tag_signature_data["verified_tags"] / tag_signature_data["total_tags"]
        
        if verified_ratio < 0.5:
            issues.append(f"Less than half of recent release tags are signed ({verified_ratio:.0%})")
        
        if tag_signature_data["unverified_tags"] > 0:
            issues.append(f"{tag_signature_data['unverified_tags']} recent tags have problematic signatures")
    else:
...
def check_signed_commits(
    ...
    if repo_dir:
        try:
            ...
            has_signed_commits = (
                commit_signature_data["verified_commits"] > 0 or
                tag_signature_data["verified_tags"] > 0 or
                commit_signing_requirement["requires_commit_signing"]
            )
            ...
            signature_data = {}
            if commit_signature_data["total_commits"] > 0:
                commit_verified_percent = (commit_signature_data["verified_commits"] / 
                                         commit_signature_data["total_commits"] * 100)
                signature_data["commit_signing_rate"] = f"{commit_verified_percent:.1f}%"
            
            if tag_signature_data["total_tags"] > 0:
                tag_verified_percent = (tag_signature_data["verified_tags"] / 
                                      tag_signature_data["total_tags"] * 100)
            ...
            # Log results
            logger.info(f"Signed commits check for {dependency.name}: {'Found' if has_signed_commits else 'Not found'}")
            if has_signed_commits:
                logger.info(f"Signed commits for {dependency.name}: Verified commits: {commit_signature_data['verified_commits']}/{commit_signature_data['total_commits']}, Tags: {tag_signature_data['verified_tags']}/{tag_signature_data['total_tags']}")
            logger.info(f"Signed commits score for {dependency.name}: {signed_commits_score:.2f}")
            for issue in issues:
