...
def analyze_commit_frequency(repo_dir: str, months: int = 12) -> Dict[str, float]:
    ...
    try:
        ...
        for i in range(months):
            ...
            cmd = [
                "git", "rev-list", "--count", f"--since={start_date}", 
                f"--until={end_date}", "HEAD"
            ]
        ...
        if len(monthly_counts) >= 3:
            ...
            earlier = sum(monthly_counts[3:6]) / 3 if len(monthly_counts) >= 6 else recent
        ...
        if len(monthly_counts) >= 3:
            variations = [abs(monthly_counts[i] - monthly_counts[i+1]) for i in range(len(monthly_counts)-1)]
            avg_variation = sum(variations) / len(variations) if variations else 0
            stability = 1.0 - (avg_variation / (max(monthly_counts) if max(monthly_counts) > 0 else 1))
...
def analyze_release_cadence(repo_dir: str, package_data: Optional[Dict] = None) -> Dict[str, float]:
    ...
    try:
        ...
        try:
            # Get tag dates
            cmd = ["git", "for-each-ref", "--sort=-creatordate", "--format=%(creatordate:iso)", "refs/tags"]
            output = subprocess.run(
                cmd, cwd=repo_dir, check=True, capture_output=True, text=True
            ).stdout.strip()
            ...
            for line in output.split("\n"):
                if line.strip():
                    try:
                        tag_date = datetime.fromisoformat(line.replace(" ", "T").replace(" -", "-"))
            ...
            if tag_dates:
                ...
                intervals = [(tag_dates[i] - tag_dates[i+1]).days for i in range(len(tag_dates)-1)]
                if intervals:
                ...
                if tag_dates:
                    ...
                    if "average_days_between_releases" in result:
                        expected_next = tag_dates[0] + timedelta(days=result["average_days_between_releases"])
                        result["release_overdue_days"] = max(0, (datetime.now() - expected_next).days)
        ...
        if package_data and not result:
            ...
            if "time" in package_data and isinstance(package_data["time"], dict):
                ...
                if release_dates:
                    ...
                    # Calculate days between releases
                    intervals = [(release_dates[i] - release_dates[i+1]).days for i in range(len(release_dates)-1)]
                    if intervals:
                        avg_interval = sum(intervals) / len(intervals)
                        result["average_days_between_releases"] = avg_interval
                    ...
                    # Calculate expected next release date
                    if "average_days_between_releases" in result:
                        expected_next = release_dates[0] + timedelta(days=result["average_days_between_releases"])
                        result["release_overdue_days"] = max(0, (datetime.now() - expected_next).days)
...
def analyze_issue_activity(repo_path: str) -> Dict[str, float]:
    ...
    try:
        ...
        has_issue_templates = any(repo_path_obj.joinpath(path).exists() for path in issue_template_paths)
        ...
        has_codeowners = any(repo_path_obj.joinpath(path).exists() for path in codeowners_paths)
        ...
        has_maintainership = any(repo_path_obj.joinpath(path).exists() for path in maintainership_files)
...
def calculate_maintained_score(
    ...
    if "release_overdue_days" in release_data and "average_days_between_releases" in release_data:
    ...
    if "has_maintainership_info" in issue_data:
        issue_score_components.append(0.7 if issue_data["has_maintainership_info"] else 0.3)
...
def check_maintained_status(
    dependency: DependencyMetadata, repo_dir: Optional[str] = None, package_data: Optional[Dict] = None
    ...
    if repo_dir:
        try:
            ...
            maintained_score = calculate_maintained_score(commit_data, release_data, issue_data)
            
            # Add maintenance issues based on analysis
            if "average_monthly_commits" in commit_data and commit_data["average_monthly_commits"] < 1:
                maintenance_issues.append("Low commit activity (less than monthly commits)")
            
            if "commit_trend" in commit_data and commit_data["commit_trend"] < -0.2:
                maintenance_issues.append("Declining commit frequency")
            
            if "days_since_last_release" in release_data and release_data["days_since_last_release"] > 365:
                maintenance_issues.append("No release in over a year")
            
            if "release_overdue_days" in release_data and release_data["release_overdue_days"] > 0:
            ...
            # Log results
            logger.info(f"Maintenance score for {dependency.name}: {maintained_score:.2f}")
        ...
        maintenance_issues.append("No repository information available for maintenance analysis")
