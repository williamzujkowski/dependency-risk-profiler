    ...
    try:
        ...
        if settings_file.exists():
            with open(settings_file, 'r', encoding='utf-8') as f:
                ...
                if re.search(r'branches:', content) and re.search(r'protection:', content):
        ...
        if workflow_dir.exists() and workflow_dir.is_dir() and not result["has_branch_protection"]:
            for workflow_file in workflow_dir.glob("*.y*ml"):
                try:
                    with open(workflow_file, 'r', encoding='utf-8') as f:
                        ...
                        if re.search(r'branch.*protection|protect.*branch', content) and not re.search(r'check.*branch', content):
                            result["has_branch_protection"] = True
                            result["protection_type"] = f"GitHub Actions: {workflow_file.name}"
                            break
                except Exception:
                    continue
    ...
    if repo_dir:
        try:
            ...
            has_branch_protection = (
                github_protection["has_branch_protection"] or
                protection_indicators["has_protected_branches"] or
                (protection_indicators["has_code_owners"] and pr_patterns["uses_pull_requests"])
            )
            ...
            if github_protection["has_branch_protection"]:
                protection_info["protection_type"] = github_protection["protection_type"]
            
            if protection_indicators["has_protected_branches"]:
                protection_info["protected_branches"] = str(protection_indicators["protected_branches"])
            
            if protection_info:
                dependency.additional_info["branch_protection"] = str(protection_info)
            
            # Log results
            logger.info(f"Branch protection check for {dependency.name}: {'Found' if has_branch_protection else 'Not found'}")
            if has_branch_protection:
                if github_protection["has_branch_protection"]:
                    details_str = ", ".join(github_protection.get("protection_details", []))
                    logger.info(f"Branch protection details for {dependency.name}: {details_str}")
            
            logger.info(f"Branch protection score for {dependency.name}: {branch_protection_score:.2f}")
            for issue in issues:
        ...
        issues.append("No repository information available for branch protection analysis")
