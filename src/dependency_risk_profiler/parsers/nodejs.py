...
class NodeJSParser(BaseParser):
                ...
                for pkg_path, pkg_info in packages.items():
                    ...
                    if "version" in pkg_info and pkg_name not in dependencies:
                        dependencies[pkg_name] = DependencyMetadata(
                            ...
                            repository_url=pkg_info.get("repository", {}).get("url") if isinstance(pkg_info.get("repository"), dict) else None,
                        )
    ...
    def _extract_dependencies(self, deps_dict: Dict, result: Dict[str, DependencyMetadata]) -> None:
        ...
        for name, info in deps_dict.items():
            if "version" in info and name not in result:
                result[name] = DependencyMetadata(
                    ...
                    repository_url=info.get("repository", {}).get("url") if isinstance(info.get("repository"), dict) else None,
                )
