...
class RiskScorer:
    ...
    def score_dependency(self, dependency: DependencyMetadata) -> DependencyRiskScore:
        ...
        health_score = self._calculate_health_indicators_score(
            dependency.has_tests, dependency.has_ci, dependency.has_contribution_guidelines
        )
    ...
    def create_project_profile(
        self, manifest_path: str, ecosystem: str, dependencies: Dict[str, DependencyMetadata]
    ) -> ProjectRiskProfile:
    ...
    def _calculate_community_score(self, community_metrics) -> float:
        ...
        if (community_metrics.open_issues_count is not None and 
            community_metrics.closed_issues_count is not None and
            (community_metrics.open_issues_count + community_metrics.closed_issues_count) > 0):
            
            # Calculate ratio of closed issues to total issues
            total_issues = community_metrics.open_issues_count + community_metrics.closed_issues_count
    ...
    def _determine_risk_factors(
        ...
        if maintainer_score and maintainer_score > 0.5:
            if dependency.maintainer_count is not None and dependency.maintainer_count < 2:
        ...
        if version_score and version_score > 0.5:
            factors.append(f"Outdated (current: {dependency.installed_version}, latest: {dependency.latest_version})")
        ...
        if license_score and license_score > 0.5:
            if dependency.license_info:
                if dependency.license_info.category.value == "NETWORK_COPYLEFT":
                    factors.append(f"Network copyleft license ({dependency.license_info.license_id})")
                elif dependency.license_info.category.value == "COPYLEFT":
                    factors.append(f"Copyleft license ({dependency.license_info.license_id})")
                elif dependency.license_info.category.value == "COMMERCIAL":
                    factors.append(f"Commercial license ({dependency.license_info.license_id})")
                    ...
                    factors.append(f"Non-approved license ({dependency.license_info.license_id})")
        
        # Community health risk factors
        if community_score and community_score > 0.5:
            if dependency.community_metrics:
                if dependency.community_metrics.star_count is not None and dependency.community_metrics.star_count < 100:
                    factors.append(f"Low popularity ({dependency.community_metrics.star_count} stars)")
                
                if dependency.community_metrics.commit_frequency is not None and dependency.community_metrics.commit_frequency < 1:
