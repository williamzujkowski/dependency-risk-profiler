...
class HistoricalTrendAnalyzer:
            ...
            "dependencies": {
                dep.dependency.name: {
                    ...
                    "scores": {
                        ...
                        "security_policy_score": getattr(dep, "security_policy_score", 0),
                        "dependency_update_score": getattr(dep, "dependency_update_score", 0),
                        "signed_commits_score": getattr(dep, "signed_commits_score", 0),
                        "branch_protection_score": getattr(dep, "branch_protection_score", 0),
    ...
    def analyze_trends(self, project_id: str, limit: int = 10) -> Dict[str, Any]:
        ...
        improving_deps, deteriorating_deps = self._identify_changing_dependencies(dependency_trends)
    ...
    def generate_trend_visualization_data(
        ...
        if visualization_type == "overall":
            ...
            return {
                ...
                "data": {
                    "labels": [point["timestamp"] for point in trend_data["overall_score_trend"]],
                    "datasets": [
                        {
                            "label": "Risk Score",
                            "data": [point["score"] for point in trend_data["overall_score_trend"]],
                            "borderColor": "#ff6384",
            ...
            return {
                ...
                "data": {
                    "labels": [point["timestamp"] for point in trend_data["risk_distribution_trend"]],
                    "datasets": [
                        {
                            "label": "High Risk",
                            "data": [point["high"] for point in trend_data["risk_distribution_trend"]],
                            "backgroundColor": "rgba(255, 99, 132, 0.7)"
                        },
                        {
                            "label": "Medium Risk",
                            "data": [point["medium"] for point in trend_data["risk_distribution_trend"]],
                            "backgroundColor": "rgba(255, 159, 64, 0.7)"
                        },
                        {
                            "label": "Low Risk",
                            "data": [point["low"] for point in trend_data["risk_distribution_trend"]],
            ...
            return {
                ...
                "data": {
                    "labels": [point["timestamp"] for point in next(iter(trend_data["dependency_trends"].values()), [])],
                    "datasets": datasets
                },
                "options": {
                    "scales": {
    ...
    def _analyze_security_improvements(
        ...
        for ts in reversed(timestamps):
            ...
            security_improvements.append({
                ...
                "metrics": {
                    "security_policy": round(security_policy_count / total_deps * 100, 1),
                    "dependency_update": round(dependency_update_count / total_deps * 100, 1),
                    "signed_commits": round(signed_commits_count / total_deps * 100, 1),
                    "branch_protection": round(branch_protection_count / total_deps * 100, 1)
                }
            })
    ...
    def _calculate_velocity_metrics(
        ...
        velocity_metrics = {
            ...
            "dependency_churn_rate": round(
                (len(new_deps) + len(removed_deps)) / days_difference, 2
            ) if days_difference > 0 else 0,
            "update_frequency": round(
                len(updated_deps) / days_difference, 2
            ) if days_difference > 0 else 0
        }
...
def generate_trend_visualization(
    ...
    """
    analyzer = HistoricalTrendAnalyzer()
    return analyzer.generate_trend_visualization_data(project_id, visualization_type, limit)