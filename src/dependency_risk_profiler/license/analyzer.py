...
def analyze_license(dependency: DependencyMetadata, metadata: Dict) -> DependencyMetadata:
    ...
    try:
        ...
        if license_info:
            ...
            logger.info(f"Found license {license_info.license_id} ({license_info.category.value}) for {dependency.name}")
        else:
