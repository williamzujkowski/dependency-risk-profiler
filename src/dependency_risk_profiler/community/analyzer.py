...
def calculate_commit_frequency(repo_dir: str, months: int = 6) -> Optional[float]:
    ...
    try:
        ...
        date_threshold = (datetime.now() - timedelta(days=30 * months)).strftime('%Y-%m-%d')
...
def analyze_github_community_metrics(dependency: DependencyMetadata) -> DependencyMetadata:
...
def analyze_npm_community_metrics(dependency: DependencyMetadata, npm_data: Dict) -> DependencyMetadata:
    ...
    if "downloads" in npm_data:
        if isinstance(npm_data["downloads"], dict) and "last-month" in npm_data["downloads"]:
            dependency.community_metrics.downloads_count = npm_data["downloads"]["last-month"]
    ...
    if "time" in npm_data:
        if isinstance(npm_data["time"], dict):
            ...
            release_dates = {k: v for k, v in npm_data["time"].items() 
            ...
            if release_dates:
                ...
                try:
                    dependency.community_metrics.last_release_date = datetime.fromisoformat(
                        latest_release[1].replace("Z", "+00:00")
                    )
                except ValueError:
                    pass
...
def analyze_pypi_community_metrics(dependency: DependencyMetadata, pypi_data: Dict) -> DependencyMetadata:
    ...
    try:
        download_stats = fetch_json(f"https://pypistats.org/api/packages/{dependency.name}/recent")
        if download_stats and "data" in download_stats:
            if "last_month" in download_stats["data"]:
                dependency.community_metrics.downloads_count = download_stats["data"]["last_month"]
    except Exception:
        pass
...
def analyze_community_metrics(dependency: DependencyMetadata, metadata: Dict = None) -> DependencyMetadata:
