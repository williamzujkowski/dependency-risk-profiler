...
class TerminalFormatter(BaseFormatter):
    ...
    def format_profile(self, profile: ProjectRiskProfile) -> str:
        ...
        result.append(f"Overall Risk Score: {self._colored(f'{profile.overall_risk_score:.2f}/5.0', overall_risk)}")
        result.append(f"High Risk Dependencies: {self._colored(str(profile.high_risk_dependencies), self.RED)}")
        result.append(f"Medium Risk Dependencies: {self._colored(str(profile.medium_risk_dependencies), self.YELLOW)}")
        result.append(f"Low Risk Dependencies: {self._colored(str(profile.low_risk_dependencies), self.GREEN)}")
        result.append("")
    ...
    def _format_dependency_row(self, dep: DependencyRiskScore) -> str:
        ...
        installed = metadata.installed_version[:13] + ".." if len(metadata.installed_version) > 15 else metadata.installed_version
        latest = metadata.latest_version[:13] + ".." if metadata.latest_version and len(metadata.latest_version) > 15 else (metadata.latest_version or "Unknown")
        ...
        # Format maintainers
        maintainers = str(metadata.maintainer_count) if metadata.maintainer_count is not None else "Unknown"
