...
class Config:
    ...
    def _load_from_env(self) -> None:
        ...
        if "DRP_USE_COLOR" in os.environ:
            self._config["general"]["use_color"] = os.environ["DRP_USE_COLOR"].lower() in ("1", "true", "yes")
        
        if "DRP_DEBUG" in os.environ:
            self._config["general"]["debug"] = os.environ["DRP_DEBUG"].lower() in ("1", "true", "yes")
        
        # Vulnerability settings
        if "DRP_GITHUB_TOKEN" in os.environ:
            self._config["vulnerability"]["github_token"] = os.environ["DRP_GITHUB_TOKEN"]
        ...
        if "DRP_DISABLE_CACHE" in os.environ:
            self._config["vulnerability"]["disable_cache"] = os.environ["DRP_DISABLE_CACHE"].lower() in ("1", "true", "yes")
        
        if "DRP_CACHE_EXPIRY" in os.environ:
            try:
                self._config["vulnerability"]["cache_expiry"] = int(os.environ["DRP_CACHE_EXPIRY"])
            except ValueError:
                pass
    ...
    def update_from_args(self, args: Dict[str, Any]) -> None:
        ...
        if "version_weight" in args and args["version_weight"] is not None:
            self._config["scoring_weights"]["version_difference"] = args["version_weight"]
        ...
        if "enable_github_advisory" in args:
            self._config["vulnerability"]["enable_github_advisory"] = args["enable_github_advisory"]
    ...
    def get_api_keys(self) -> Dict[str, str]:
        ...
        if vuln_config.get("enable_github_advisory") and vuln_config.get("github_token"):
    ...
    def generate_sample_config(self, file_path: Union[str, Path], format: str = "toml") -> bool:
        ...
        try:
            if format.lower() == "toml":
                ...
                with open(file_path, 'w') as f:
                    ...
                    # General section
                    f.write("[general]\n")
                    f.write(f"output_format = \"{DEFAULT_CONFIG['general']['output_format']}\"\n")
                    f.write(f"use_color = {str(DEFAULT_CONFIG['general']['use_color']).lower()}\n")
                    f.write(f"debug = {str(DEFAULT_CONFIG['general']['debug']).lower()}\n\n")
