    ...
    try:
        ...
        if output_path:
            ...
            with output_path.open("w") as f:
                ...
                f.write(f"Hash ({signature_info['hash_algorithm']}): {signature_info['hash']}\n")
...
def verify_signature(
    ...
    try:
        ...
        if current_hash != original_hash:
            logger.error(f"Hash mismatch. Original: {original_hash}, Current: {current_hash}")
            return False
...
def main() -> int:
    ...
    parser.add_argument(
        "artifact",
        help="Path to the artifact to sign or verify"
    )
    ...
    parser.add_argument(
        "--output",
        "-o",
        help="Path to write the signature file"
    )
    
    parser.add_argument(
        "--log-file",
        help="Path to the signing log file"
    )
