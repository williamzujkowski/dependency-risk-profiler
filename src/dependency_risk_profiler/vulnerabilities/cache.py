...
This module provides functions to store and retrieve vulnerability data from a disk-based
cache, reducing the need for frequent network calls to vulnerability APIs.
"""
import json
import logging
import os
...
class VulnerabilityCache:
    ...
    def __init__(self, cache_dir: Optional[Path] = None, expiry: int = DEFAULT_CACHE_EXPIRY):
        """Initialize the vulnerability cache.
        
        Args:
            cache_dir: Directory to store cache files. Defaults to ~/.dependency_risk_profiler/vuln_cache
    ...
    def get(self, package_name: str, ecosystem: str) -> Optional[Tuple[List[Dict[str, Any]], float]]:
        ...
        """
        logger.info(f"Attempting to retrieve cache data for {package_name} ({ecosystem})")
        ...
        try:
            ...
            if not all(k in cache_data for k in ['data', 'timestamp']):
                logger.warning(f"Malformed cache entry for {package_name}: missing required fields")
                return None
            ...
            logger.info(f"Serving vulnerability data for {package_name} from disk cache (created: {datetime.fromtimestamp(timestamp).isoformat()})")
    ...
    def set(self, package_name: str, ecosystem: str, data: List[Dict[str, Any]]) -> bool:
    ...
    def clear(self, package_name: Optional[str] = None, ecosystem: Optional[str] = None) -> int:
