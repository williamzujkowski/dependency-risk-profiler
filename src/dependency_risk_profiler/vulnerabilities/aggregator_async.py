...
class AsyncOSVSource(OSVSource):
    ...
    async def get_vulnerabilities_async(self, package_name: str, ecosystem: str) -> List[Dict[str, Any]]:
...
class AsyncNVDSource(NVDSource):
    ...
    async def get_vulnerabilities_async(self, package_name: str, ecosystem: str) -> List[Dict[str, Any]]:
        ...
        params = {
            "keywordSearch": f"{cpe_prefix}{package_name}",
            "resultsPerPage": 100
        }
...
class AsyncGitHubAdvisorySource(GitHubAdvisorySource):
    ...
    async def get_vulnerabilities_async(self, package_name: str, ecosystem: str) -> List[Dict[str, Any]]:
        ...
        # Extract vulnerability data
        vulnerabilities = response_data.get("data", {}).get("securityVulnerabilities", {}).get("nodes", [])
...
async def aggregate_vulnerabilities_for_package_async(
    ...
    if cached:
        ...
        return _update_dependency_with_vulnerabilities(dependency, vulnerabilities), vulnerabilities
    ...
    updated_dependency = _update_dependency_with_vulnerabilities(dependency, unique_vulnerabilities)
...
async def aggregate_vulnerability_data_async_impl(
    ...
    """
    logger.info(f"Aggregating vulnerability data for {len(dependencies)} dependencies asynchronously")
    ...
    for i in range(0, len(dependency_names), batch_size):
        ...
        results_raw = await asyncio.gather(*tasks, return_exceptions=True)
        # Type check for mypy
        results: List[Union[Tuple[DependencyMetadata, List[Dict[str, Any]]], BaseException]] = results_raw
...
def aggregate_vulnerability_data_async(
    ...
    try:
        ...
        logger.error(f"Error in asynchronous vulnerability aggregation: {e}", exc_info=True)
