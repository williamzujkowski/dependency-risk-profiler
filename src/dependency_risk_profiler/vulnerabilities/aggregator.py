...
class VulnerabilitySource:
    ...
    def __init__(self, name: str, base_url: str, enabled: bool = True, timeout: int = 10):
    ...
    def get_vulnerabilities(self, package_name: str, ecosystem: str) -> List[Dict[str, Any]]:
...
def _update_dependency_with_vulnerabilities(
    ...
    for vuln in vulnerabilities:
        ...
        for ref in references:
            if isinstance(ref, str) and any(term in ref.lower() for term in ["exploit", "poc", "proof-of-concept"]):
                dependency.has_known_exploits = True
                break
        
        # Also check summary and details
        for field in ["summary", "details"]:
            if field in vuln and any(term in vuln[field].lower() for term in ["exploit", "poc", "proof-of-concept"]):
                dependency.has_known_exploits = True
                break
    ...
    if vulnerabilities:
        ...
        recent_vulnerabilities = [v for v in vulnerabilities if v.get("published", "") >= ninety_days_ago]
