...
class TestSetupBuildEnvironment:
    ...
    def test_setup_build_environment(
        self, mock_datetime: Mock, mock_time: Mock
    ) -> None:
        """Verify setup_build_environment returns expected environment values."""
        # Setup mocks
...
class TestFetchLatestCode:
    ...
    def test_fetch_latest_code_success(self, mock_run: Mock) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result = fetch_latest_code(
                "https://github.com/org/repo.git", "main", temp_dir
            )
...
class TestBuildPackage:
    ...
    def test_build_package_success(
        self, mock_run: Mock, mock_shutil: Mock, mock_os: Mock
    ) -> None:
...
class TestSignArtifacts:
    ...
    def test_sign_artifacts_success(self, mock_sign_artifact: Mock) -> None:
        """Verify sign_artifacts signs each artifact and returns results."""
        # Setup mocks
        mock_sign_artifact.return_value = {
        ...
        with tempfile.NamedTemporaryFile() as temp_file1:
            with tempfile.NamedTemporaryFile() as temp_file2:
                ...
                result = sign_artifacts(
                    artifacts, "build-123", SigningMode.TEST, "log.txt"
                )
                ...
                for artifact in artifacts:
                    assert (
                        mock_sign_artifact.call_args_list[
...
class TestCreateBuildManifest:
    ...
    def test_create_build_manifest_success(self, mock_run: Mock) -> None:
        """Verify create_build_manifest creates artifact manifest data."""
        # Setup mocks
        mock_process = Mock()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            signatures = {
                artifact_path: {"timestamp": "2023-01-15T12:00:00Z", "mode": "TEST"}
            }
            build_env = {
                "BUILD_ID": "build-123",
            ...
            result = create_build_manifest(
                artifacts, signatures, build_env, manifest_path
            )
    ...
    def test_create_build_manifest_failure(self) -> None:
        """Verify create_build_manifest raises BuildError on exception."""
...
class TestRunBuildProcess:
    ...
    @patch(
        "dependency_risk_profiler.secure_release.release_build.tempfile.TemporaryDirectory"
    )
    @patch(
        "dependency_risk_profiler.secure_release.release_build.setup_build_environment"
    )
    ...
    @patch(
        "dependency_risk_profiler.secure_release.release_build.create_build_manifest"
    )
    ...
    def test_run_build_process_success(
        ...
        with tempfile.TemporaryDirectory() as output_dir:
            ...
            # The manifest path might be in a temp dir; check the filename.
