...
class TestDependencyInstallation:
    ...
    def test_install_packages_failure(self, mock_run):
        ...
        pip_path = "/path/to/venv/bin/pip"
        requirements_file = "/path/to/requirements.txt"
        mock_run.return_value = Mock(returncode=1, stderr="Error: could not find package")
...
class TestPipdeptreeExecution:
    ...
    def test_run_pipdeptree_success(self, mock_run):
        ...
        mock_output = json.dumps([
            {
                "package": {"key": "requests", "package_name": "requests", "installed_version": "2.28.1"},
                "dependencies": [
                    {"package": {"key": "urllib3", "package_name": "urllib3", "installed_version": "1.26.12"}},
                    {"package": {"key": "charset-normalizer", "package_name": "charset-normalizer", "installed_version": "2.1.1"}}
                ]
            }
        ])
    ...
    def test_run_pipdeptree_failure(self, mock_run):
        ...
        # Arrange
        python_path = "/path/to/venv/bin/python"
        mock_run.return_value = Mock(returncode=1, stderr="Error: pipdeptree not installed")
...
class TestDependencyAnalysis:
    ...
    def test_analyze_python_transitive_dependencies(
        self, mock_run_pipdeptree, mock_install_pipdeptree, mock_install_packages, mock_create_venv
    ):
        ...
        mock_pipdeptree_output = [
            ...
            {
                "package": {"key": "flask", "package_name": "Flask", "installed_version": "2.2.2"},
                "dependencies": [
                    {"package": {"key": "werkzeug", "package_name": "Werkzeug", "installed_version": "2.2.2"}},
                    {"package": {"key": "jinja2", "package_name": "Jinja2", "installed_version": "3.1.2"}}
                ]
            }
        ]
    ...
    @patch('dependency_risk_profiler.transitive.analyzer_enhanced.analyze_python_transitive_dependencies')
    ...
    @patch('dependency_risk_profiler.transitive.analyzer_enhanced.analyze_python_transitive_dependencies')
    @patch('dependency_risk_profiler.transitive.analyzer_enhanced.analyze_pyproject_toml_dependencies')
    @patch('dependency_risk_profiler.transitive.analyzer_enhanced.analyze_pipfile_lock_dependencies')
...
@patch('dependency_risk_profiler.transitive.analyzer_enhanced.extract_python_dependencies_enhanced')
def test_analyze_transitive_dependencies_enhanced(mock_extract_python):
    ...
    assert result["requests"].transitive_dependencies == {"urllib3", "charset-normalizer"}
