...
def test_flit_pyproject_parsing(flit_pyproject_toml):
    ...
    if "pydantic[email]" in dependencies:
        ...
        assert dependencies["pydantic"].installed_version.startswith(">=") or "[email]" in dependencies["pydantic"].installed_version
    
    # Check for test dependencies if they exist
    test_deps = [name for name, dep in dependencies.items() 
               if ("project.optional-dependencies.test" == dep.additional_info.get("section") or
                  "test" in dep.additional_info.get("groups", ""))]
...
def test_workspace_cargo_toml_parsing(workspace_cargo_toml):
    ...
    assert dependencies["serde"].installed_version in ["1.0", "workspace = true", "unknown"]
    
    assert "tokio" in dependencies
    assert dependencies["tokio"].installed_version in ["1.28", "workspace = true", "unknown"]
    
    # Check for build dependencies
    assert "cc" in dependencies
    assert dependencies["cc"].installed_version == "1.0"
    build_deps = [name for name, dep in dependencies.items() 
                 if "build_dependency" in dep.additional_info or
                   "build-dependencies" in dep.additional_info.get("section", "")]
    ...
    dev_deps = [name for name, dep in dependencies.items() 
               if "dev_dependency" in dep.additional_info or
                 "dev-dependencies" in dep.additional_info.get("section", "")]
...
def test_extract_version_methods():
    ...
    try:
        ...
        assert parser._parse_dependency_string("requests>=2.25.0") == ("requests", ">=2.25.0")
        ...
        assert parser._parse_dependency_string("pydantic[email]>=1.9.0") == ("pydantic[email]", ">=1.9.0")
        ...
        assert parser._extract_poetry_version({"git": "https://github.com/example/repo.git"}) == "git:https://github.com/example/repo.git"
        ...
        assert parser._extract_cargo_version({"git": "https://github.com/example/repo.git"}) == "git:https://github.com/example/repo.git"
        assert parser._extract_cargo_version({"git": "https://github.com/example/repo.git", "tag": "v1.0"}) == "git:https://github.com/example/repo.git@v1.0"
    finally:
