...
def test_staleness_score_calculation():
    ...
    for last_updated, expected_score in test_cases:
        ...
        assert result == expected_score, f"Failed for {last_updated}, got {result} instead of {expected_score}"
...
def test_maintainer_score_calculation():
    ...
    for maintainer_count, expected_score in test_cases:
        ...
        assert result == expected_score, f"Failed for {maintainer_count}, got {result} instead of {expected_score}"
...
def test_version_difference_score_calculation():
    ...
    for installed, latest, expected_score in test_cases:
        ...
        assert result == expected_score, f"Failed for {installed} -> {latest}, got {result} instead of {expected_score}"
...
def test_health_indicators_score_calculation():
    ...
    for has_tests, has_ci, has_contribution_guidelines, expected_score in test_cases:
        result = scorer._calculate_health_indicators_score(has_tests, has_ci, has_contribution_guidelines)
        
        if expected_score is None:
            assert result is None, f"Expected None for ({has_tests}, {has_ci}, {has_contribution_guidelines})"
        else:
            # Allow for small floating point differences due to division
            assert abs(result - expected_score) < 0.01, f"Failed for ({has_tests}, {has_ci}, {has_contribution_guidelines}), got {result} instead of {expected_score}"
...
def test_risk_level_determination():
    ...
    for score, expected_level in test_cases:
        ...
        assert result == expected_level, f"Failed for score {score}, got {result} instead of {expected_level}"
...
def test_total_score_calculation_with_all_metrics():
    ...
    assert score.total_score <= max_score, f"Score {score.total_score} should not exceed max_score {max_score}"
    assert score.risk_level == RiskLevel.CRITICAL, "Risk level should be CRITICAL for a dependency with all risk factors"
    
    # The total score should be close to max score since all factors are at maximum risk
    assert score.total_score >= max_score * 0.8, "Score should be close to max_score with all risk factors at maximum"
...
def test_total_score_calculation_with_partial_metrics():
    ...
    # Assert
    assert score.total_score > 0, "Score should be greater than 0 even with minimal data"
...
def test_project_profile_creation():
    ...
    high_risk_count = sum(1 for dep in scored_dependencies 
                        if dep.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL])
    medium_risk_count = sum(1 for dep in scored_dependencies 
                         if dep.risk_level == RiskLevel.MEDIUM)
    low_risk_count = sum(1 for dep in scored_dependencies 
                       if dep.risk_level == RiskLevel.LOW)
    
    overall_score = sum(dep.total_score for dep in scored_dependencies) / len(scored_dependencies)
    ...
    assert profile.high_risk_dependencies == 2, "Should identify 2 high/critical risk dependencies"
    assert profile.medium_risk_dependencies == 1, "Should identify 1 medium risk dependency" 
...
def test_license_score_calculation():
    ...
    for license_info, expected_score in test_cases:
        ...
        assert result == expected_score, f"Failed for {license_info}, got {result} instead of {expected_score}"
...
def test_regression_tzinfo_handling():
    ...
    dep = DependencyMetadata(
        name="tz-test-package",
        installed_version="1.0.0",
        last_updated=tz_aware_date
    )
    ...
    assert hasattr(score, "staleness_score"), "Score should have staleness_score attribute"
...
def test_regression_version_parsing_edge_cases():
    ...
    for installed, latest, expected_score in test_cases:
        try:
            ...
            assert result >= 0.0 and result <= 1.0, f"Score should be between 0.0 and 1.0 for {installed} -> {latest}"
            assert abs(result - expected_score) < 0.26, f"Score for {installed} -> {latest} should be approximately {expected_score}"
...
def test_regression_risk_factors_without_data():
    ...
    # Create a dependency with minimal data
    minimal_dep = DependencyMetadata(
        name="minimal-package",
        installed_version="1.0.0"
    )
    ...
    missing_latest = DependencyMetadata(
        name="missing-latest",
        installed_version="1.0.0",
        last_updated=datetime.now() - timedelta(days=400)  # Stale but no latest version
    )
    ...
    try:
        ...
        assert "Not updated in" in " ".join(missing_latest_score.factors), "Should include staleness risk factor"
        assert not any("Outdated" in factor for factor in missing_latest_score.factors), \
        ...
        assert "Outdated" in " ".join(missing_dates_score.factors), "Should include version difference risk factor"
        assert not any("Not updated in" in factor for factor in missing_dates_score.factors), \
        ...
        assert "No dependency update tools found" in " ".join(partial_security_score.factors), \
            "Should include risk factor for known false security metric"
        assert "Security policy status unknown" in " ".join(partial_security_score.factors), \
...
def test_regression_weight_normalization():
    ...
    assert abs(result - expected_score) < 0.01, f"Score {result} should be close to expected {expected_score}"
    assert result > 4.9, "Score should be high with high-risk factors having normal weights"
...
def test_scoring_performance_sla():
    ...
    assert p95_time < 2.0, f"95th percentile scoring time {p95_time}ms exceeds SLA of 2ms"
...
def test_project_profile_performance_sla():
    ...
    for i in range(num_dependencies):
        ...
        dep = DependencyMetadata(
            name=f"dep-{i}",
            installed_version="1.0.0",
            latest_version="2.0.0" if is_high_risk else "1.1.0" if is_medium_risk else "1.0.1",
            last_updated=datetime.now() - timedelta(days=400 if is_high_risk else 100 if is_medium_risk else 10),
    ...
    # Assert
    assert elapsed_time < 50.0, f"Project profile creation took {elapsed_time}ms, exceeding SLA of 50ms for 100 dependencies"
    assert len(profile.dependencies) == num_dependencies, f"Profile should include all {num_dependencies} dependencies"
    
    # Count risk levels to verify proper profile creation
    high_count = sum(1 for dep in profile.dependencies if dep.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL])
    medium_count = sum(1 for dep in profile.dependencies if dep.risk_level == RiskLevel.MEDIUM)
    low_count = sum(1 for dep in profile.dependencies if dep.risk_level == RiskLevel.LOW)
...
def test_risk_factor_determination_performance_sla():
    ...
    dep = DependencyMetadata(
        ...
        transitive_dependencies=["dep1", "dep2", "dep3", "dep4", "dep5", "dep6", "dep7", "dep8", "dep9", "dep10",
    ...
    for _ in range(num_iterations):
        ...
        version_score = scorer._calculate_version_difference_score(dep.installed_version, dep.latest_version)
        health_score = scorer._calculate_health_indicators_score(dep.has_tests, dep.has_ci, dep.has_contribution_guidelines)
        ...
        transitive_score = scorer._calculate_transitive_score(dep.transitive_dependencies)
        security_policy_score = scorer._calculate_security_policy_score(dep.security_metrics)
        dependency_update_score = scorer._calculate_dependency_update_score(dep.security_metrics)
        signed_commits_score = scorer._calculate_signed_commits_score(dep.security_metrics)
        branch_protection_score = scorer._calculate_branch_protection_score(dep.security_metrics)
        ...
        determination_times.append((time.time() - factors_start_time) * 1000)  # Convert to ms
    
    # Assert
    avg_time = sum(determination_times) / len(determination_times)
    max_time = max(determination_times)
    
    assert avg_time < 0.5, f"Average risk factor determination time {avg_time}ms exceeds SLA of 0.5ms"
    assert max_time < 1.0, f"Maximum risk factor determination time {max_time}ms exceeds SLA of 1.0ms"
    
    # Also verify that we get a comprehensive list of risk factors
    assert len(factors) >= 10, f"Should identify at least 10 risk factors, but only found {len(factors)}"
...
def test_fuzzing_version_difference_score():
    ...
    grammar = {
        ...
        'semver': ['<digit>.<digit>.<digit>', '<digit>.<digit>.<digit>-<prerelease>', '<digit>.<digit>.<digit>+<build>'],
        'partial_semver': ['<digit>.<digit>', '<digit>'],
        'range_version': ['^<semver>', '~<semver>', '>=<semver>', '<=<semver>', '<semver> - <semver>'],
    ...
    assert len(failures) == 0, f"Found {len(failures)} failures during fuzzing: {failures[:3]}"
    
    # Check function behavior - should always return a float between 0-1
    for result in results:
        output = result['output']
        assert isinstance(output, float), f"Expected float, got {type(output)} for input {result['input']}"
        assert 0 <= output <= 1, f"Expected value between 0-1, got {output} for input {result['input']}"
...
def test_fuzzing_staleness_score():
    ...
    test_cases = [
        ...
        (now.replace(tzinfo=pytest.importorskip("zoneinfo").ZoneInfo("UTC")), 0.0),  # Now with timezone
        (
            (now - timedelta(days=400)).replace(tzinfo=pytest.importorskip("zoneinfo").ZoneInfo("UTC")), 
            1.0
    ...
    for dt, expected_score in test_cases:
        ...
        # Assert
        assert result == expected_score, f"Failed for {dt_desc}: expected {expected_score}, got {result}"
        
        # Also verify type and range
        if result is not None:  # Some edge cases might return None
            assert isinstance(result, float), f"Expected float, got {type(result)} for {dt_desc}"
            assert 0 <= result <= 1, f"Expected value between 0-1, got {result} for {dt_desc}"
...
def test_fuzzing_risk_level_determination():
    ...
    grammar = {
        ...
        'boundary_score': ['1.24', '1.25', '1.26', '2.49', '2.5', '2.51', '3.74', '3.75', '3.76'],
    ...
    def test_func(input_str):
        ...
        scorer = RiskScorer(max_score=5.0)
        return scorer._determine_risk_level(score).name if score is not None else "ERROR"
    ...
    for result in results:
        ...
        if output != "ERROR":  # Skip the None input case
            assert output in valid_levels, f"Expected valid risk level, got {output} for input {result['input']}"
...
def test_logging_information_completeness():
    ...
    def logged_calculate_staleness(self, last_updated):
        ...
        logger.debug(f"Calculated staleness score {result} for last_updated {last_updated}")
        return result
    ...
    assert any("Calculated staleness score" in msg for msg in log_messages), \
        "Missing log for staleness score calculation"
    
    # Should log the risk factors determination
    assert any("Determined risk factors" in msg for msg in log_messages), \
        "Missing log for risk factors determination"
    
    # Should include the dependency name in logs
    assert any(dep.name in msg for msg in log_messages), \
    ...
    for log in logs:
        if "Determined risk factors" in log["message"]:
            ...
            assert "[" in log["message"] and "]" in log["message"], \
...
def test_logging_decision_points():
    ...
    def determine_risk_level(score, max_score, thresholds):
        ...
        mock_logger.debug(f"DECISION_POINT: Risk level determination for score {score} (normalized: {normalized_score:.2f})")
        ...
        # Log the outcome
        mock_logger.debug(f"DECISION_OUTCOME: Determined risk level {result.name} based on threshold {thresholds.get(result)}")
    ...
    assert len(decision_logs) == len(test_cases), \
    ...
    assert len(outcome_logs) == len(test_cases), \
        f"Expected {len(test_cases)} decision outcome logs, got {len(outcome_logs)}"
    
    # Verify logs contain all risk levels
    risk_levels_in_logs = [log["message"].split("risk level ")[1].split(" ")[0] for log in outcome_logs]
    
    for level in [RiskLevel.LOW.name, RiskLevel.MEDIUM.name, RiskLevel.HIGH.name, RiskLevel.CRITICAL.name]:
